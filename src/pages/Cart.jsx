...
let total=0;
  ...
  return (
    <div className='w-full bg-gray-100'>
     {
      ...
      <div className='w-full h-full bg-white px-4 col-span-4'>
          ...
          <div>
              {
                 products.map((item)=>(
                  ...
                  <div className='w-full flex items-center justify-center gap-6'>
                    <div className=' w-1/5'>
                    <img className='w-full h-44 object-contain' src={item.image} alt='ProductsImage' />
                    </div>
                    <div className='w-4/5'>
                      <h2 className=' font-semibold text-lg'> {item.title} </h2>
                      <p className='text-sm'>{item.description.substring(0,200)}</p> 
                      <p className=' text-base'>  Unit price <span> Rs {item.price} </span> </p>
                    ...
                    <div className='w-1/5'>
                      <p className='text-lg font-titleFont font-semibold'>
                       Rs {(item.price*item.quantity).toFixed(2)} </p>
                      </div>
                </div>
                </div>
                 )
          ...
          <div className='w-full py-2'>
            <button onClick={()=>dispatch(resetCart())} className='bg-red-500 px-10 py-2 hover:bg-red-600 active:bg-red-500
      ...
      <div className='w-full h-52 flex flex-col items-center justify-center p-4 bg-white col-span-1'>
        <div>
          <p className=' flex gap-2 items-start  text-sm'> <span>
            <CheckCircleIcon className=' bg-white text-green-500 rounded-full'/> </span>
          {"Your order qualifiess for free shipping.Choose this option at checkout See details..."} </p>
        </div>
        <div>
          <p className='font-semibold px-10 py-1 flex items-center gap-2 justify-between'>Total: 
          <span className='text-lg font-bold'>RS {totalprice}</span> </p>
          <button className='font-medium font-titleFont 
          text-base bg-gradient-to-tr from-yellow-400 to-yellow-200 border hover:from-yellow-300  hover:to-yellow-500
          border-yellow-500 hover:border-yellow-700 active:bg-gradient-to-bl
          active:from-yellow-400 active:to-yellow-500 duration-200 py-1.5 rounded-md mt-4'>Proceed to Pay</button>
    ...
    transition={{delay:0.5,duration:0.5}}
     ...
     <div className='w-96 flex flex-col p-4 bg-white items-center rounded-md shadow-lg'>
      <h1 className=' font-titleFont text-xl font-bold'>Your cart feels lonely</h1>
      <p className=' text-sm text-center'>Your cart livess to serve.fill it</p>
      <Link to='/'>
      <button className=' mt-6 bg-yellow-400 rounded-md cursor-pointer
      hover:bg-yellow-500 active:bg-yellow-700 px-8 py-2 font-titleFont font-semibold text-lg'>
